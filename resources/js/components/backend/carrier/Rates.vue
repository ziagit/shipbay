    ...
    <md-table md-sort="name" md-sort-order="asc" md-card>
      ...
      <div class="table-content">
        ...
        <md-table-row v-for="rate in rates.data" :key="rate.id">
          ...
          <md-table-cell>{{ rate.citycodes[0].postal_code }}</md-table-cell>
          <md-table-cell>{{ rate.citycodes[1].postal_code }}</md-table-cell>
          ...
          <md-table-cell>{{ rate.carriers[0].first_name }}</md-table-cell>
...
export default {
  ...
  methods: {
    search() {
      axios
        .get("admin/search-rate", { params: { keywords: this.keywords } })
        .then((res) => {
          this.rates = res.data;
        })
        .catch((err) => {
