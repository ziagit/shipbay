    ...
    <md-card class="no-shadow-bordered">
      ...
      <md-card-content>
        ...
        <div v-show="!dataLoading">
          ...
          >
            <a class="payment-card" @click="checkoutTogal = true">Add new card</a>
          </md-empty-state>
        </div>
      </md-card-content>
      <md-card-actions>
        <div class="process-order">
          ...
          <div v-if="isCardAdded && pendingOrder">
            <md-button v-if="paymentStatus" @click="targetRoute()"
              >Process order</md-button
            >
            <div v-else>
              <div class="alert-success">Your cart added.</div>
              <md-button v-if="!cardSubmiting" @click="chargeCustomer()"
                >Pay now</md-button
              >
